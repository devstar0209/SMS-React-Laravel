...
export function* updateProfile(action){
    try{
        const response = yield call(ApiHelper.post, resources.teacher.profile.update, action.payload);
...
export function* profileDetail(action){
    try{
        ...
        yield put({
                type: PROFILE.TEACHER_SEND_PROFILE_DETAIL,
                payload: data
            });
...
export default function* PROFILESaga() {
	yield takeLatest(PROFILE.TEACHER_REQUEST_PROFILE_DETAIL, profileDetail);
	yield takeLatest(PROFILE.TEACHER_REQUEST_PROFILE_UPDATE, updateProfile);
}
